...
const getSignalDirection = (data, timeframes) => {
    ...
    let direction = 'MIXED';
    if (totalBuys === totalSignals && totalBuys > 0)
        direction = 'BUY';
    else if (totalSells === totalSignals && totalSells > 0)
        direction = 'SELL';
    return {
        direction: direction,
...
const rankSignalData = (symbol, data) => {
    if (Object.keys(data).length < ALL_TIMEFRAMES.length)
        return null;
...
app.get('/api/scan', async (req, res) => {
    try {
        ...
        for (const symbol of symbolsToScan) {
            ...
            tfResults.forEach(r => {
                if (r)
                    signalData[r.tf] = { signal: r.signal };
                else
                    complete = false;
            });
            if (complete) {
                ...
                if (ranked) {
                    results.push(ranked);
                    if (ranked.signal.includes('BUY'))
                        marketStats.bullish++;
                    else if (ranked.signal.includes('SELL'))
                        marketStats.bearish++;
                    else
                        marketStats.neutral++;
                }
